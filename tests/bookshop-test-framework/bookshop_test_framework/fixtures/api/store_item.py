...
async def session_store_item(store_items_api: StoreItemApi, session_product: Product, session_manager_user: User,
    ...
    create_store_item_response = await store_items_api.create(store_item, auth=UserApi.get_auth(session_manager_user))
    ...
    http_task_group.create_task(store_items_api.delete(store_item.id, auth=UserApi.get_auth(session_manager_user)))


@pytest.fixture(scope="session")
async def session_get_store_item_response(store_items_api: StoreItemApi, session_manager_user: User,
    ...
    """
    return await store_items_api.get(session_store_item.id, auth=UserApi.get_auth(session_manager_user))


@pytest.fixture(scope="session")
async def session_get_all_store_items_response(store_items_api: StoreItemApi,
    ...
    """
    return await store_items_api.get_all(auth=UserApi.get_auth(session_manager_user))


@pytest.fixture(scope="session")
async def session_update_store_item_response(store_items_api: StoreItemApi, session_manager_user: User,
                                             session_store_item: StoreItem) -> ClientResponse:
    ...
    return await store_items_api.update(session_store_item.id, attributes_to_update,
                                        auth=UserApi.get_auth(session_manager_user))


@pytest.fixture
async def created_store_item_response(store_items_api: StoreItemApi, generated_store_item: StoreItem,
    ...
    if create_store_item_response.ok:
        ...
        http_task_group.create_task(
            store_items_api.delete(created_store_item_id, auth=UserApi.get_auth(session_manager_user)))
