...
def soft_check_response_status_code(response: ClientResponse,
                                    expected_status_codes: Union[int, Iterable[int]] = codes.ok) -> bool:
...
async def soft_check_response_body_is_json(response: ClientResponse) -> bool:
    ...
    return expect(response_json is not None, ApiTestResult(
        check_message="Check response has body in JSON format",
        method=response.method,
        endpoint=response.url.path
    ))
...
async def soft_check_response_json_is_not_empty(response: ClientResponse) -> bool:
    ...
    """
    response_json = await response.json()
    return expect(response_json, ApiTestResult(
        check_message="Check response has not empty body JSON",
        method=response.method,
        endpoint=response.url.path
    ))


async def soft_check_response_json_attributes(response: ClientResponse, expected_attributes: dict, *,
                                              ignore_actual_fields: Optional[Collection[str]] = None) -> bool:
    """Check response has body and body is json
